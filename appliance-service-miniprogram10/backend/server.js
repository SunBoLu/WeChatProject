...
// 工具函数：snake_case 对象字段 → camelCase（适配微信小程序前端对订单/用户等数据的驼峰式引用）
...
function toCamelCase(obj) {
  ...
  // 保留 Date、Buffer 等特殊对象类型，避免被错误地转成空对象 {}
...
app.get('/api/orders/:id', async (req, res) => {
  ...
  try {
    ...
    res.json({ ...toCamelCase(order), publisher: toCamelCase(publisher), provider: toCamelCase(provider) });
  } catch (err) {
    console.error(err);
...
app.put('/api/users/:userId', async (req, res) => {
  const { userId } = req.params;
  const { name, avatar, role, is_banned, warning_count, address, phone, latitude, longitude, payment_qr_code, info_collected, is_first_login } = req.body;
  try {
    const fields = [];
    const params = [];
    let idx = 1;
...
app.post('/api/orders/:id/verify-payment', async (req, res) => {
  ...
  try {
    ...
    if (!result.rows[0]) return res.status(404).json({ error: '订单不存在' });
    res.json({ success: true, message: '支付验证成功，订单发布成功', order: toCamelCase(result.rows[0]) });
  } catch (err) {
    console.error(err);
...
app.get('/api/users', async (req, res) => {
  const { role, keyword, phone, openid } = req.query;
  try {
    ...
    if (phone && phone !== '') {
      ...
      // 精确匹配 openid（用于微信登录时查找已有用户）
      query += ` AND openid = $${idx++}`;
      params.push(openid);
    } else {
      if (role && role !== 'all') {
        query += ` AND role = $${idx++}`;
