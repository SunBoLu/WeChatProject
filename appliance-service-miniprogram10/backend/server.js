...
// -------------------- 数据库 & 服务保活（每天 12:00） --------------------
/**
 * 极轻量的数据库保活查询：只消耗一次 SELECT 1，几乎零带宽、零 CPU。
 * 同时在 PostgreSQL 端会刷新 idle 连接的活跃时间，避免被清理。
 */
async function keepAlive() {
  const tag = '[keep-alive]';
  try {
    const result = await pool.query('SELECT 1 AS ping');
    if (result && result.rows && result.rows[0]) {
      console.log(`${tag} 数据库连接正常 (${new Date().toISOString()})`);
      return true;
    }
    console.warn(`${tag} 查询返回为空`);
    return false;
  } catch (err) {
    console.error(`${tag} 数据库连接失败：`, err.message);
    return false;
  }
}

// 每天中午 12:00（服务器本地时区）执行一次 keep-alive 查询
// cron 格式：秒 分 时 日 月 星期（0-6，0=周日）
const keepAliveTask = cron.schedule('0 0 12 * * *', () => {
  keepAlive();
}, {
  scheduled: true,
...
// 服务启动时立刻执行一次 keep-alive，验证数据库可用并保持连接
keepAlive();

// 暴露健康检查接口，外部监控或手动检查可访问 GET /api/health
